...
### D020: Smart Price verdict — inclusive band boundaries, `unknown` fallbacks, whole-map memo key
**Date:** 2026-05-16
**Context:** Three design micro-decisions inside `src/lib/comps.ts` + the surfaces that consume it. Each is small, but each has a defensible alternative, so they're recorded together. (1) The verdict's relationship to the band edges — is "matching the cheapest comp" fair or below? (2) When the comp set degenerates (no comps, or every comp's current bid is 0), the panel could still render zeros and the badge could still claim "below market" — both options were considered. (3) `compPriceBand` is called from `VehicleCard`, `BidPanel`, and `CompPanel`; the memo key could be just the target's bids + its comps' bids, or the whole `bidsByVehicle` map.
**Decision:** (1) Verdict boundaries are inclusive: `price < low → 'below'`, `low ≤ price ≤ high → 'fair'`, `price > high → 'above'`. (2) `compPriceBand` returns `null` when `findComps` is empty *or* when every comp price is ≤ 0; `smartPriceVerdict` returns `'unknown'` for a null band or a non-positive candidate price. (3) Every consumer memoizes on `[vehicle, bidsByVehicle]` — the whole map, not a precomputed subset.
**Rationale:** (1) Inclusive matches the buyer's mental model and mirrors D011's buy-now ceiling — exact-equal boundaries should succeed at face value, not feel "almost." (2) A band of zeros isn't a signal; rendering it would mark every lot with a real price as "above market" and undermine the trust thesis the badge is built around. The `'unknown'` verdict + "No recent comps" copy is honest about absence of signal. (3) Computing the precise dependency subset (target's own bids + each comp's bids) requires running `findComps` *outside* the memo to identify comps before keying — which defeats the memo. Keying on the whole map costs a referential identity check; the band recomputes only when `BidProvider` actually commits a bid (D019's ref-mirror makes commits atomic), so unrelated mutations don't churn.
**Caveat:** The whole-map key means *any* bid commit re-runs comps for every visible card on the inventory grid. With 200 vehicles × 3-comp lookups × O(pool) filter, that's still well under a frame on the M7 verification hardware, but if the dataset grew an order of magnitude the right move would be precomputing comp-id lists at module load and keying card memos on a slice of the map.
